...
ipcMain.handle('open-file-dialog', async (event, _arg) => {
  ...
  const properties = process.platform !== 'darwin' ? ['openFile'] : ['openFile', 'openDirectory']
  ...
  const file = await dialog.showOpenDialog({
    ...
    buttonLabel: 'Open',
    filters: [
      { name: 'MRI Videos', extensions: ['avi', 'npy'] },
    ],
  ...
  if (!file) {
    ...
    if (file.canceled) {
      ...
      const outputDir = path.resolve(`${userDataPath_temp}/${filename}_converted.mp4`);
      ...
      const ffmpegPromise = new Promise((resolve, reject) => {
        ...
        .on('error', (errFfmpeg) => {
          ...
          console.error('================== Failed opening file ==================');
...
ipcMain.handle('open-multi-files-dialog', async (_event, _arg) => {
  ...
  const properties = process.platform !== 'darwin' ? ['openFile', 'multiSelections'] : ['openFile', 'openDirectory', 'multiSelections']
...
ipcMain.handle('get-video-metadata', async (event, data) => {
  console.log('======================= Retrieving video\'s metadata ======================');
  const filePath = data;
  ...
  console.log('=================== Finished Retrieving video\'s metadata ==================');
...
ipcMain.handle('make-single-prediction', async (event, filepath) => {
  ...
  const unetPretrainPath = path.resolve(__dirname + '/../extra/prediction_module/unet3.h5')
  const checkColNumPretrainPath = path.resolve(__dirname + '/../extra/prediction_module/check_col_num.h5')
  const classifyPretrainPath = path.resolve(__dirname + '/../extra/prediction_module/classify5.h5')
  const options = {
    mode: 'text',
    pythonOptions: ['-u'],
    args: [filepath, unetPretrainPath, checkColNumPretrainPath, classifyPretrainPath],
  };

  const predictionModulePath = path.resolve(__dirname + '/../extra/prediction_module/model.py')
...
async function makeMultiPrediction(subfolderDir, eventForReplying) {
  ...
  */
  ...
  eventForReplying.reply('response-get-multiple-prediction-result', JSON.stringify(returnObj));
